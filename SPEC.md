...
**Research persistence:** findings are saved to `.codebutler/research/` as individual MD files. The Researcher maintains an index in its conversation, decides what's worth persisting vs what's throwaway. Persisted research is committed to git and merges with PRs — knowledge accumulates across threads. The Researcher checks existing research before searching again.
