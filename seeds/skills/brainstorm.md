...
4. Design the Thinker panel: decide how many (2–6), craft a unique system prompt for each (persona + focus area + domain context), assign a model to each from the brainstorm pool
5. Post the panel design in the thread so the user knows what's coming
6. Call BrainstormFanOut with the per-Thinker configs
